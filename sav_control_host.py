...
class RunEmulation():
    ...
    def _start_metric_monitor(self,interval=1):
        """
        clear metric history and start metric monitor (a thread)
        @param interval: the interval of metric monitor (check resource usage every interval seconds)
        """
    ...
    def _get_result(self):
        ...
        return ret
    def start(self,monitor_overlap_sec=5):
...
def run(args):
    ...
    if action is not None:
        ...
        match action:
            case 'start':
                result = json.dump(result,run_emulation.start(),indent=2)
            case 'stop':
                result = "SAVOP stop"
            case 'restart':
